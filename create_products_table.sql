...
CREATE POLICY "Store owners can manage their products" ON products
    FOR ALL USING (
        ...
        EXISTS (
            ...
            AND profiles.role IN ('admin', 'store_admin')
        )
    );
...
('Contoh Produk 1', 'Deskripsi produk contoh', 100000, 'Elektronik', 10, (SELECT id FROM profiles WHERE role = 'store_admin' LIMIT 1), 'https://via.placeholder.com/300x300'),
('Contoh Produk 2', 'Deskripsi produk contoh 2', 250000, 'Fashion', 5, (SELECT id FROM profiles WHERE role = 'store_admin' LIMIT 1), 'https://via.placeholder.com/300x300')
